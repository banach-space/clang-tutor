...
Example Clang plugins for C and C++ - based on **Clang 23**
...
declarations](https://github.com/llvm/llvm-project/blob/release/23.x/clang/include/clang/AST/DeclCXX.h#L253)
...
```bash
# Build the plugin
export Clang_DIR=<installation/dir/of/clang/23>
...
## Installing Clang 23 On Mac OS X
On Darwin you can install Clang 23 and LLVM 23 with
...
upgrade it to Clang 23 and LLVM 23 like this:
...
sudo apt-add-repository "deb http://apt.llvm.org/jammy/ llvm-toolchain-jammy-23 main"
sudo apt-get update
sudo apt-get install -y llvm-23 llvm-23-dev libllvm23 llvm-23-tools clang-23 libclang-common-23-dev libclang-23-dev libmlir-23 libmlir-23-dev
```
This will install all the required header files, libraries and tools in
`/usr/lib/llvm-23/`.

## Building Clang 23 From Sources
Building from sources can be slow and tricky to debug. It is not necessary, but
might be your preferred way of obtaining LLVM/Clang 23. The following steps
...
```bash
cd <build/dir>
cmake -DCT_Clang_INSTALL_DIR=<installation/dir/of/clang/23> <source/dir/clang-tutor>
make
```

The `CT_Clang_INSTALL_DIR` variable should be set to the root of either the
installation or build directory of Clang 23. It is used to locate the
...
not bundled with LLVM 23 packages, but you can install it with **pip**:
```bash
...
[DiagnosticEngine](https://github.com/llvm/llvm-project/blob/release/23.x/clang/include/clang/Basic/Diagnostic.h#L191)
...
[FixItHint](https://github.com/llvm/llvm-project/blob/release/23.x/clang/include/clang/Basic/Diagnostic.h#L70)
API.
[SourceLocation](https://github.com/llvm/llvm-project/blob/release/23.x/clang/include/clang/Basic/SourceLocation.h#L86)
